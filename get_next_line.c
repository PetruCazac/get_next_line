...
/*   Updated: 2023/05/11 14:56:16 by pcazac           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_newln(int fd, int *count, char *static_buffer, char **return_line)
{
	...
	if (i > 0)
		*count = *count + BUFFER_SIZE;
	if (i < 0 || (i == 0 && *count == 0))
		return (ft_bzero(static_buffer, BUFFER_SIZE), *return_line = NULL);
	if (temp_buffer[line_end(temp_buffer)] == '\n' || (i == 0 && *count != 0))
	{
		if (!ft_calloc(return_line, count, static_buffer))
			return (NULL);
...
char	*read_buffer(char *static_buffer, char **return_line)
{
	...
	*return_line = malloc((BUFFER_SIZE + 1) * sizeof(char));
	if (!(*return_line))
		return (NULL);
	ft_bzero(*return_line, BUFFER_SIZE);
	i = -1;
	while (++i <= BUFFER_SIZE)
		*(*return_line + i) = 0;
	i = -1;
...
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE < 1 || fd > 10240 || fd < 0)
		return (NULL);
	while (static_buffer[count] && static_buffer[count] != '\n')
		count++;
	if (count <= BUFFER_SIZE && static_buffer[count] == '\n')
		read_buffer(static_buffer, &return_line);
	else
		read_newln(fd, &count, static_buffer, &return_line);
